    ...
    <div class="ubn ub-ver wd-100">
      <el-table :data="tableData" stripe >
        ...
        <el-table-column prop="pwdWrong" label="密码错误" width="100px"></el-table-column>
         <el-table-column prop="loginLock" label="登录限制">
...
export default {
  ...
  setup () {
    ...
    const modifyClc = ({ id, memo, name, secretKey, status }) => {
      state.dialogData = { id, memo, name, secretKey, status }
      state.addNew = false
      state.showD = true
    }
    const confirmClc = () => {
    ...
    const bindRoleClc = () => {
      roleCheckPost('accountUpdate', state.dialogData).then(res => {
        if (res) {
